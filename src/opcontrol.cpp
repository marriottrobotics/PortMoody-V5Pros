...
void updateLoader(){
  ...
  //Controller.ButtonB.pressed( loaderSpinDown );
	...
	if(joystick.get_digital(DIGITAL_R2)){
			loader.move(127);
      loaderMode = 0;
  }else if(joystick.get_digital(DIGITAL_R1)){
			loader.move(-127);
      loaderMode = 0;
...
//Brake Stuff --------------------------------------------------------------
void updateBrakes(){
	if(joystick.get_digital_new_press(DIGITAL_L2)){
		brake();
	}
}
...
void updateCatipult(){
    if(catipult.get_power() > 1.0 && catipult.is_stopped()){
		...
		if(joystick.get_digital_new_press(DIGITAL_Y)){
			fire();
		}
}
//End Catipult Stuff ----------------------------------------------------------
